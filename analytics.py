...
# ============================================================
# Буферизация событий (опционально, для высокой нагрузки)
# ============================================================
...
class AnalyticsBuffer:
    ...
    async def flush(self):
        ...
        self.queue.clear()
        self.last_flush = asyncio.get_event_loop().time()
        # Вставляем все события одной транзакцией
        try:
...
# Глобальный экземпляр буфера (создать после подключения к БД)
...
class AnalyticsMiddleware(BaseMiddleware):
    """Middleware, логирующий появление пользователя и его активность."""
    async def __call__(
        self,
        ...
        if user and not user.is_bot:
            # Регистрируем или обновляем пользователя (без обновления last_activity при каждом действии)
            ...
            # Логируем сам факт действия (если нужно, можно добавить событие 'activity')
            # Не делаем это, чтобы не плодить миллион событий. Вместо этого будем логировать только конкретные действия.
...
async def _log_event(user_id: int, event_type: str, target_id: int = None,
    ...
    if analytics_buffer:
        ...
        # Падение в случае, если буфер не инициализирован – логируем напрямую
        try:
            await db._conn.execute(
                """
                INSERT INTO analytics_events (user_id, event_type, target_id, target_type, metadata)
...
async def get_retention(cohort_days_ago: int, after_days: int) -> float:
    """
    Retention: доля пользователей, которые совершили действие через `after_days` дней
    после своего первого визита (cohort = cohort_days_ago дней назад).
    Например, D1 retention: cohort_days_ago=1, after_days=1.
    Возвращает процент (0-100).
    """
    # Пользователи, которые впервые появились ровно cohort_days_ago дней назад
    ...
    # Из них активные через after_days дней
...
async def get_section_popularity(days: int = 30) -> list:
    """Популярность разделов: количество событий открытия раздела."""
...
async def get_top_items(item_type: str, days: int = 30, limit: int = 10) -> list:
    """Топ-10 самых просматриваемых сущностей (мобов, ресурсов и т.д.)."""
    event_map = {
        'mob': 'view_mob',
...
async def get_db_stats() -> dict:
    """Возвращает размер таблиц и количество записей (для мониторинга)."""
